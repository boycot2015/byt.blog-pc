...
export default {
  ...
  methods: {
    newtime() {
      ...
      const date = timeFormat(new Date().getTime(), 'YYYY年MM月DD日')
      this.datetime = date
    },
    weeks() {
      const weekAarry = ['日', '一', '二', '三', '四', '五', '六']
      const week = weekAarry[new Date().getDay()]
      this.week = '星期' + week
    },
    hdnongli() {
      const madd = [12]
      ...
      const dd = D.getDate()
      //   const ww = D.getDay()
      //   const ss = parseInt(D.getTime() / 1000)
      if (yy < 100) yy = '19' + yy
      function showCal() {
        const nonglitime = GetLunarDay(yy, mm, dd)
        return nonglitime
      }
      this.nongli = showCal()
    },
